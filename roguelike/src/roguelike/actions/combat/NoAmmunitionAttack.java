...
public class NoAmmunitionAttack extends Attack {
	...
	public NoAmmunitionAttack(int baseDamage, RangedWeapon weapon) {
		super("does not have any " + weapon.ammunitionType().name + "s ready with which to shoot the %s", baseDamage, weapon);
	}
	...
	protected boolean onPerform(Action action, Actor target) {
		...
		Game.current().displayMessage(actor.doAction("has no %ss readied.", ((RangedWeapon) weapon).ammunitionType().name), SColor.GRAPE_MOUSE);
		return false;
	}
